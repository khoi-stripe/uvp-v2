...
function RolesPermissionsContent({ sandboxMode, setSandboxMode, layoutVersion = "v2", customRoles, setCustomRoles, reduceCounts = false, mergedCanCannot = false }: {
    ...
    <>
      <div className="flex-1 min-h-0 flex flex-col gap-6 overflow-hidden">
        ...
        <div className={`flex-1 min-h-0 min-w-0 flex flex-col overflow-hidden ${useInvertedColors ? '' : 'bg-[#F5F6F8] rounded-xl p-2'}`}>
              ...
              <p className="text-[13px] text-[#596171] leading-[19px] tracking-[-0.15px]">
                ...
                {isV6 ? (
                  <button
                    ...
                    className="text-[#635BFF] hover:text-[#533AFD] transition-colors"
                  >
                    View permissions
                  </button>
                ) : (
          ...
          <div
            className={`flex flex-col overflow-hidden transition-[flex,opacity] duration-500 ${(!isV6 || showPermissionsPanel) ? 'flex-1 min-w-0 opacity-100' : 'flex-[0] w-0 opacity-0 pointer-events-none'}`}
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-2 min-h-0 pt-[18px]" style={rolesMaxWidth != null ? { maxWidth: rolesMaxWidth } : undefined}>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    >
                      ...
                      <span>{showPermissions ? 'Hide role permissions' : 'View role permissions'}</span>
                    </button>
                  </div>
...
function TeamAndSecurityPageInner() {
  ...
  }, [activeTab, layoutVersion, teamSecurityEnabled, use14px, singleRoleSelect, reduceCounts, mergedCanCannot, wireframe]);
    ...
    <div className={`h-screen flex bg-white min-w-[1024px] ${use14px ? 'use-14px' : ''}`}>
      ...
      <div className={`flex-1 flex flex-col overflow-hidden ${wireframe ? 'px-8 pb-6' : 'px-8 pb-6'}`}>
        ...
        <div className={`flex-1 min-h-0 flex flex-col gap-6 pt-5 max-w-[1400px] mx-auto w-full ${activeTab === "roles" ? 'overflow-hidden' : 'overflow-auto'}`}>
          ...
          {/* Tab content */}
          {activeTab === "roles" ? (
            <RolesPermissionsContent sandboxMode={sandboxMode} setSandboxMode={setSandboxMode} layoutVersion={layoutVersion} customRoles={customRoles} setCustomRoles={setCustomRoles} reduceCounts={reduceCounts} mergedCanCannot={mergedCanCannot} />
          ) : (

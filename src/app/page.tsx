...
function RolesPermissionsContent({ sandboxMode, setSandboxMode, layoutVersion = "v2", customRoles, setCustomRoles, compactTabMode = false, reduceCounts = false, mergedCanCannot = false }: {
  ...
  // v6: panel starts open, closeable via X, "View permissions" always an active link
  const [showPermissionsPanel, setShowPermissionsPanel] = useState(layoutVersion === "v6");
  const useTabLayout = compactTabMode;
  const useEyeToggle = isV5 && useTabLayout;
    ...
    <>
      <div className="flex-1 min-h-0 flex flex-col gap-6 overflow-hidden">
        ...
        <div className={`flex-1 min-h-0 min-w-0 flex flex-col overflow-hidden ${useInvertedColors ? '' : 'bg-[#F5F6F8] rounded-xl p-2'}`}>
          ...
          <div
            className={`flex flex-col overflow-hidden transition-[flex,opacity] duration-500 ${(isV6 ? showPermissionsPanel : (isV5 ? (!useTabLayout || showPermissionsPanel) : true)) ? 'flex-1 min-w-0 opacity-100' : 'flex-[0] w-0 opacity-0 pointer-events-none'}`}
